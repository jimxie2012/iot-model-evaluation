...
def train_all(model_kind , sample_name):

    sample_file = "%s/%s.csv"%(g_ds_sample_path,sample_name)
    if model_kind == "h2o":
        print("train h2o ",sample_file)
        train(sample_file , sample_name )
    if model_kind == "svm":
        print("train svm " , sample_name)
        train_svm( sample_name  )
        print("train done ",sample_name)

def main():
    if len(sys.argv) != 2:
        print("please run with model kind (h2o or svm ), for example: python train.py svm")
    else:
        model_kind = sys.argv[1].strip().lower()
        ...
        for fi in csv_lsit:
            ...
            if sample_name.split("-")[-1] == '0':
                ...
                train_all(model_kind,sample_name)
